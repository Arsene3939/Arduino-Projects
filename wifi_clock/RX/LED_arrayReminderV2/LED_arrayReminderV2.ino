...
void setup(){
  ...
  if (setTime)Rtc.SetDateTime(compiled);
...
void without_rtc_mode_detect(){
  now = Rtc.GetDateTime();
  week=redate(now.Month()*100+now.Day(),now.Year());
  if(long_press(2))mode=7;
  if(long_press(1)){
